...
// 1. TỔNG GIÁ GỐC CỦA SẢN PHẨM (chưa giảm)
$originalSubtotal = 0;

// 2. TỔNG GIÁ SAU GIẢM TRÊN TỪNG SẢN PHẨM
$productDiscountedSubtotal = 0;

// 3. TÍNH TỔNG GIẢM TRÊN SẢN PHẨM
...
<td>
    ...
    <!-- NÚT HÀNH ĐỘNG -->
    <div class="mt-4">
        <a href="<?php echo APP_URL; ?>/Home/orderHistory" class="btn btn-secondary">⬅ Quay lại</a>
<?php
      // Nút ĐÃ NHẬN HÀNG khi đang giao
    if ($order['status'] === 'shipping') { ?>
        <a href="<?= APP_URL ?>/Order/confirmReceived/<?= $order['id'] ?>" 
    ...
    // Nút HỦY ĐƠN khi đang ở pending hoặc approved
    if (in_array($order['status'], ['pending', 'approved'])) { 
        $message = ($order['transaction_info'] === 'dathanhtoan')
            ? "Đơn đang được chuẩn bị, bạn có chắc chắn muốn hủy đơn không?"
            : "Bạn có chắc chắn muốn hủy đơn này?";
    ?>
        <a href="<?= APP_URL ?>/Order/cancelOrder/<?= $order['id'] ?>" 
